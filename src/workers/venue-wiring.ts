 ...
 *                             && METHOD_COSTS[v] !== undefined     (blocker 2)
 *
 ...
 * fails to compile in `EXCHANGE_RESOLVERS` and in `METHOD_COSTS` until the new
 ...
 * ── KRAKEN TODAY ──
 *
 * WIRED, and NOBODY FLIPPED ANYTHING. Blocker 1 closed when
 * `workers/exchange-kraken.ts` landed and `EXCHANGE_RESOLVERS.kraken` became
 * real. Blocker 2 closed when the rate-limiter session gave `METHOD_COSTS` its
 * `kraken` row -- a decaying account counter, a second PER-PAIR counter, and a
 * cancel priced by the order's age, which is why it needed `MethodCost` to be a
 * function of the call rather than a number keyed by its name.
 *
 * This paragraph is the receipt for the previous one's claim. The version of
 * this file before that session said the gap "closes on its own the moment that
 * row lands", and it did: no boolean was edited, no test was updated to permit
 * it, and the only change here was renaming the table this reads. That is the
 * property the derivation exists to have, and it has now been exercised once.
 */
 ...
 *
 * A runtime presence check rather than a type-level one. It was written that way
 * because `METHOD_COSTS`'s predecessor was DECLARED total while its object
 * literal was missing the `kraken` key -- the type lied, and that mismatch was
 * the open `tsc` error at `rate-limited.ts:196`, left standing on purpose as
 * entry 90 step (d)'s forcing function.
 *
 * The type no longer lies: every `ExchangeId` has a row. The runtime check is
 * KEPT ANYWAY, because it costs nothing and it is what makes this file correct
 * during the window a future venue is half-added -- which is the state Kraken was
 * in for five entries, and the state the next venue will be in too.
 *
...
export function hasRateLimitCostModel(exchange: ExchangeId): boolean {
  return METHOD_COSTS[exchange] !== undefined;
}

/**
 * Whether a bot may be created on, and traded on, this venue today.
 *
 * DERIVED, never declared. There is no boolean to flip: the day `METHOD_COSTS`
 * grew its `kraken` row, `hasRateLimitCostModel("kraken")` became true and
 * Kraken became wired in the same commit, with nothing else to remember. That is
 * the whole point of the change -- the previous hand-maintained row had to be
 * flipped by someone who noticed, and the test that would have told them was the
 * only thing standing between "wired" and "a green build, a 201, and a TypeError
 * at the first trade".
 *
 ...
 * gives a bot that cannot authenticate. Kraken was exactly the first case until
 * the rate-limiter session closed it.
 */
...
export function describeUnwiredExchange(exchange: ExchangeId): string {
  ...
  if (!hasRateLimitCostModel(exchange)) {
    missing.push(
      "no rate-limit cost model (no METHOD_COSTS row, so its calls cannot be " +
